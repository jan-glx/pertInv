...
dt = rbindlist(lapply( seq_len(n_folds), function (fold) {
  ...
  dt = rbind(res_SSq("intercept_only",matrix(rep(1,nrow(guide_matrix),ncol=1))),
             res_SSq("+capture",cbind(1,capture)),
...
Y.dt = data.table(melt(Y))
setnames(Y.dt, c("cell","gene","log1p"))

dt2 <- rbind(dt2,
      merge(Y.dt,counts.dt)[,
     .(method="poisson noise",
       `R^2`=
         var(log2(1+rpois(1000,fitdistrplus::fitdist(count,"nbinom", method="mme")$estimate[["mu"]])))/var(log1p)
     ), by=gene],fill=TRUE)
...
dt2 <- rbind(dt2,
             counts.dt[,{
               ...
               ret <-  data.table(
                 ...
                 method="poisson noise 2")
               ret[,`R^2`:=1-res_SSq/res_SSq_0]
               ret}, by=gene],fill=TRUE)

dt2[, .(`mean R^2`=mean(`R^2`)), by=.(method)]
