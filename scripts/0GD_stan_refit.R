...
dat$beta_p_R_r <-  dat$alpha_p_R_r*(dat$n_r-1)
dat$beta_var_X_g <-  dat$alpha_var_X_g*0.1



ii <- 2
mm[[ii]] <-  stan_model_builder("stan_lib/sim_params_with_guide.stan")
fit_mc[[ii]] <- sampling(mm[[ii]], data = dat, warmup=1999, chains=1)
e[[ii]] <- extract(fit_mc[[ii]])

dat  <-  c(dat, lapply(e[[ii]], drop))
...
fit_mc[[ii]] <- sampling(mm[[ii]], data = dat) # does not fit correctly
e[[ii]] <- extract(fit_mc[[ii]])
mean(e[[ii]]$ll_test) # -469.7167
