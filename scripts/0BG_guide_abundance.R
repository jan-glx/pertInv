...
# -----
guides_per_cell_dt <- data.table(unique_guides_per_cell=rowSums(guide_matrix))

figure("guides per cell",
       ggplot(guides_per_cell_dt,aes(x=as.factor(unique_guides_per_cell)))+geom_bar(aes(y=..count../sum(..count..)))+
         xlab("# unique guides per cell (detected)")+ylab("relative frequency"), height=2.5,width=4.5
)
