...
i=3
YY <- Y_adj[,i]
#XX <- cbind(model.matrix(~I(CDR^2)+I(CDR^3)+CDR+total_counts_scaled+batch,data=covariates.dt),
 #           Y_adj[,-i])
XX <-Y_adj[,-i]
...
pb = txtProgressBar(min = 0, max = ncol(Y_adj), initial = 0)
res = list()
for (i in seq_len(ncol(Y_adj))) {
  ...
  res[[i]] <-ICP(XX, YY, ExpInd
                 ...
                 showCompletion = FALSE,stopIfEmpty=TRUE) #hiddenICP(XX, YY, ExpInd, intercept = TRUE)
