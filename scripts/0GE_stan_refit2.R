...
#lp_R_term_tm1 <-  t(extract(fit_vb_tm1, pars="lp_R_terms")[[1]][1,,])
...
for (t in seq_len(n)){
  ...
  #lp_R_term_t <- t(extract(fit_vb_t, pars="lp_R_terms")[[1]][1,,])
  #p_acceptance[t] <- 1/(1+exp(lp_R_term_tm1[i]-lp_R_term_t[i]))
  #p_acceptance[t] <- exp(lp_R_term_t[i])
  lp_D_X1_Z1 <- lp__vb(fit_vb_t, extract(fit_vb_t))
  lp_D_X0_Z0 <- lp__vb(fit_vb_tm1, extract(fit_vb_tm1))
  lp_D_X1_Z0 <- lp__vb(fit_vb_tm1, extract(fit_vb_t))
  lp_D_X0_Z1 <- lp__vb(fit_vb_t, extract(fit_vb_tm1))

  p_acceptance[t] <- m
  accpeted <- runif(1)< exp(p_acceptance[t])
  if (accpeted) {
    cat(":")
    #lp_tm1 <- lp_t
    #fit_vb_tm1 <- fit_vb_t
