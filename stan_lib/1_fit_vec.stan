...
model {
  ...
  target += lognormal_lpdf(sd_log_p_R_r | -1, 0.1); // prior of sd of log abundance of sgRNAs
  target += lognormal_lpdf(sd_mu_X | -1, 0.1);  // prior of sd of mean log expression of genes
  target += lognormal_lpdf(sd_E | -1, 0.1); // prior of sd of log size factors
  target += normal_lpdf(mu_log_sd_X_g | -1, 0.1); // prior of mean log variance of log expression of genes
  target += lognormal_lpdf(sd_log_sd_X_g | -1, 0.1);  // prior of sd of log variance of log expression of genes
  target += lognormal_lpdf(sd_gRNA_effects | -3, 0.1); // prior of sd of sgRNA effects

  // non-hirachical prior parameters
  target += normal_lpdf(mu_X | 0, 1);
  target += beta_lpdf(p_D_given_R[1] | 100, 1);
  target += beta_lpdf(p_D_given_R[2] | 10, 1000);
